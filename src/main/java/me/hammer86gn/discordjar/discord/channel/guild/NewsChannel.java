...
public class NewsChannel implements IChannel {
    ...
    public NewsChannel(String id, String guild_id, String name, int position, JsonArray permission_overwrites, boolean nsfw, String topic, String last_message_id, String parent_id) {
        ...
        if (discordFriendlyName.toCharArray().length > 100) {
            throw new InvalidParameterException("GuildNewsChannel names can only be a maximum of 100 characters");
        }
        if (discordFriendlyName.toCharArray().length < 2) {
            throw new InvalidParameterException("GuildNewsChannel names can only be a minimum of 2 characters");
        }
        if (topic.toCharArray().length > 1024) {
            throw new InvalidParameterException("GuildNewsChannel topics can only be a maximum of 1024 characters");
        }
